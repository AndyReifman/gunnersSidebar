...
def getStats(html, comp):
    ...
    for row in stats.findAll("tr", {"class": ["odd", "even"]}):
        ...
        name = (
            row.find("td", {"class": "hauptlink"}).find_all("a", href=True)[0].getText()
        )
...
def parseStats():
    ...
    champions_league_website = requests.get(
        champions_league, headers={"User-Agent": "*"}
    )
    eflCupWebsite = requests.get(eflCup, headers={"User-Agent": "*"})
    ...
    for x in range(0, 5):
        ...
        if newLine == "":
            ...
            try:
                ...
                pass
            body += (
                "|[" + player + "](http://www.arsenal.com/men/players/" + temp + ")|"
            )
            body += newLine
    return body
